...
export class Api {
    ...
    fetchPools({ pageSize, page, orderBy, orderDirection}) {
        return this.client.query({
            query: gql`
                query {
                    pools(first:${pageSize}, skip: ${(page - 1) * pageSize}, orderBy: ${orderBy || 'liquidity'}, orderDirection:${orderDirection || 'desc'}, where: {publicSwap: true}){
                        id
                        name
                        symbol
                        totalSwapVolume
            ...
            .then(response => response.data.pools.map(item => {
                return {
                    ...item,
