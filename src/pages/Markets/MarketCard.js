...
import { InputGroupFormControl as FormControl, InputGroup, InputGroupText, InputGroupPrepend} from "../../components/Form";
...
const MarketCard = props => {
    ...
    const columns = [
        ...
        },
        {
            ...
            formatter: (cellContent, row) => {
                ...
                return (<Text color={percent > 0 ? 'success' : percent < 0 ? 'danger' : 'text1'}>{percent > 0 ? '+' : ''}{percent?.toFixed(2)}%</Text>)
            },
            sort: true,
        },
        {
        ...
        },
        {
            ...
            formatter: (cellContent, row, rowIndex, { markets }) => {
                const market =
                    markets &&
                    markets.find(m => row.pair.toUpperCase() === marketToString(m.currencyPair));
                return (
                    <div className="d-flex flex-column align-items-stretch align-items-lg-start justify-content-center w-100">
                        <TradeButton onClick={() => {
                            ...
                            props.history.push(`/trade/spot?base=${market?.currencyPair?.base?.toLowerCase()}&quote=${market?.currencyPair?.quote?.toLowerCase()}`)
                        }}>
                            {t('trade')}
                        </TradeButton>
                    </div>
    ...
    const allMarkets = useMemo(() => {
        let data = Object.values(props.markets).filter(market => !query ? market : JSON.stringify(market).toLowerCase().includes(query));
        return sortedData(data, sort);
    }, [props.markets, query, sort]);

    const btcMarkets = useMemo(() => {
        let data = Object.values(props.markets).filter(market => /btc$/.test(market.pair.toLowerCase()) && (!query || JSON.stringify({b: market.base, q: market.quote}).toLowerCase().includes(query)));
        return sortedData(data, sort);
    }, [props.markets, query, sort])

    const ethMarkets = useMemo(() => {
        let data = Object.values(props.markets).filter(market => /eth$/.test(market.pair.toLowerCase()) && (!query || JSON.stringify({b: market.base, q: market.quote}).toLowerCase().includes(query)));
        return sortedData(data, sort);
    }, [props.markets, query, sort])

    const usdcMarkets = useMemo(() => {
        let data = Object.values(props.markets).filter(market => /usdc$/.test(market.pair.toLowerCase()) && (!query || JSON.stringify({b: market.base, q: market.quote}).toLowerCase().includes(query)));
        return sortedData(data, sort);
    }, [props.markets, query, sort])
                ...
                <Row>
                    <HeaderCol xs={12} className={'d-flex flex-column-reverse flex-lg-row align-items-stretch align-items-lg-start justify-content-start justify-content-lg-between'}>
                        ...
                        <CustomInputGroup className={'w-auto mb-lg-0'} bg={'darker'}>
                            ...
                            <FormControl id="PoolsSearch" placeholder={t('search')} onChange={e => setQuery(e.target.value.toLowerCase())}/>
                        </CustomInputGroup>
                    </HeaderCol>
                    <Col xs={12}>
