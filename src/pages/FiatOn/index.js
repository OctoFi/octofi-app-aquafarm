...
class FiatOn extends Component {
				...
				return {
					...
					selected: {
						...
						crypto: {
							...
							currency:
								cryptoRes.data.response.find(
									(item) => item.symbol === process.env.REACT_APP_TRANSAK_CRYPTO_SYMBOL
								) || null,
						},
						fiat: {
							...prevState.selected.fiat,
							currency:
								fiatRes.data.response.find(
									(item) => item.symbol === process.env.REACT_APP_TRANSAK_FIAT_SYMBOL
								) || null,
						},
					},
					loading: false,
	...
	onSelect = async (value, type) => {
		...
		let cryptoValue = "";
		if (
			(this.state.selected.crypto.currency !== null || type === "crypto") &&
			(this.state.selected.fiat.currency !== null || type === "fiat")
		) {
			try {
				const prices = await this.api.get("price", {
					type: "BUY",
								...
								<Row>
										...
										selected.crypto.value > 0 && (
											<PriceCol xs={12}>
												<Row>
													<Col xs={12} md={6}>
														<PriceText>
															{selected.fiat.currency && selected.crypto.currency
																? `${t("exchange.rate")}: 1 ${
																		selected.crypto.currency.symbol
																  } = ${(conversionRate > 0
																		? 1 / conversionRate
																		: 1 / selected.crypto.currency.priceUSD
																  ).toFixed(4)} ${selected.fiat.currency.symbol}`
																: ""}
														</PriceText>
													</Col>
													<Col xs={12} md={6}>
														<PriceText>
															{t("exchange.youWillGet")}: {selected.crypto.value}{" "}
															{selected.crypto.currency.symbol}
														</PriceText>
													</Col>
												</Row>
									...
									<Col xs={12} className={"d-flex align-items-center justify-content-center"}>
										<BuyButton
											...
											disabled={
												(this.checkLimits() && this.props.web3.account) ||
												this.state.priceLoading
											}
											variant={!this.checkLimits() ? "primary" : "outline-primary"}
										>
											{this.state.priceLoading ? (
												<Loading
													width={24}
													...
													id={"fiat-on-price"}
													active
													color={this.checkLimits() ? "primary" : "#fff"}
												/>
											...
											) : !selected.fiat.value || !selected.crypto.value ? (
