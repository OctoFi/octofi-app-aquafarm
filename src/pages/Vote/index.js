											...
											>
												<VoteContent
													...
													dangerouslySetInnerHTML={{
														__html: md.render(selectedProposal.msg.payload.body),
													}}
												/>
											</Card>
										</Col>
										...
										<Col xs={12} className={"gutter-b"}>
											...
											>
												<div className="d-flex flex-column">
													<Table>
														<Thead
															style={{ maxHeight: 1, opacity: 0, visibility: "hidden" }}
														>
															<tr>
																<td>user</td>
																<td>vote</td>
															...
															{result
																? showMore ||
																  (result.hasOwnProperty("votes") &&
																		Object.keys(result.votes || {}).length < 10)
																	? Object.values(result.votes).map((vote, index) => {
																			return (
																				<VoteItem
																					key={index}
																					address={vote?.address}
																					vote={
																						selectedProposal?.msg?.payload
																							?.choices[
										...
										<div className="d-flex flex-column justify-content-start">
											...
											<ListItem title={t("author")}>
												<div className="d-flex align-items-center">
													<ModifiedJazzicon
														address={selectedProposal && selectedProposal.address}
													/>
													<AuthorLink
														href={`https://etherscan.io/address/${
															selectedProposal && selectedProposal.address
														}`}
														target={"_blank"}
														rel={"noopener noreferrer"}
													>
														{selectedProposal && selectedProposal.address.slice(0, 6)}...
														{selectedProposal && selectedProposal.address.slice(-4)}
													</AuthorLink>
												</div>
											</ListItem>
											...
											<ListItem title={"IPFS"}>
												<AuthorLink
													href={`https://ipfs.io/ipfs/${
														selectedProposal && selectedProposal.authorIpfsHash
													}`}
													target={"_blank"}
													rel={"noopener noreferrer"}
												>
											...
											<ListItem title={t("startDate")}>
												<InfoText fontWeight={400}>
													{selectedProposal &&
														moment(selectedProposal.msg.payload.start * 1e3).format(
															"YYYY/MM/DD hh:mm"
														)}
												</InfoText>
											</ListItem>
											<ListItem title={t("endDate")}>
												<InfoText fontWeight={400}>
													{selectedProposal &&
														moment(selectedProposal.msg.payload.end * 1e3).format(
															"YYYY/MM/DD hh:mm"
														)}
												</InfoText>
											</ListItem>
													...
													return (
														<ResultRow>
															<div className="d-flex align-items-center justify-content-between mb-2">
																<ResultTitle>{shorten(choice, "choice")}</ResultTitle>
