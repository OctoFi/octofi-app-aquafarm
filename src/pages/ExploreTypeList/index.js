...
const tokenSetsColumns = [
		...
		formatter: (cellContent, row) => (
			<div className="d-flex align-items-center">
				{row.components.map((c) => {
					return <CellText className={`mr-lg-4 ml-2 font-size-base`}>{c.symbol}</CellText>;
				})}
			</div>
		),
	},
	{
		...
		text: "NATURAL UNITS",
		formatter: (cellContent, row) => <CellText className="font-weight-bold">{row.natural_unit}</CellText>,
	},
	{
		dataField: "unit_shares",
		text: "UNIT SHARES",
		formatter: (cellContent, row) => <CellText className="font-weight-bold">{row.unit_shares}</CellText>,
	},
	{
