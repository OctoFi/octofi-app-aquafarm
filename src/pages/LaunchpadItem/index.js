...
const LaunchpadItem = props => {
    ...
    const listingPrice = useMemo(() => {
        if(presale?.listingPrice) {
            return new BigNumber(presale?.listingPrice?.toString() || 0).dividedBy(10 ** baseToken?.decimals).toString()
        }
        return 0;
    }, [presale, baseToken])
    ...
    const filledPercent = useMemo(() => {
        if(presale?.hardCap && presale?.status?.totalBaseCollected) {
            ...
            const totalBaseCollected = new BigNumber(presale?.status?.totalBaseCollected?.toString() || 0).dividedBy(10 ** baseToken?.decimals)
    ...
    const depositedToken = useMemo(() => {
        ...
        if(buyState?.baseDeposited && listingPrice) {
            const baseDeposited = new BigNumber(buyState?.baseDeposited?.toString() || 0).dividedBy(10 ** (baseToken?.decimals || 18))
    ...
    const purchaseHandler = () => {
        ...
        contract.estimateGas.userDeposit(...args, { value: presale?.presaleInEth ? new BigNumber(investAmount).times(10 ** baseToken?.decimals).toString() : 0 })
            .then(res => {
                contract.userDeposit(...args, {
                    gasLimit: calculateGasMargin(res),
                    value: presale?.presaleInEth ? new BigNumber(investAmount).times(10 ** baseToken?.decimals).toString() : 0
                }).then(() => {
                    setInvestAmount('');
    ...
    const withdrawBaseToken = () => {
        ...
        contract.userWithdrawBaseTokens()
            .then(res => {
                toast.success('Withdrawn was successful')
                return true;
            })
                    ...
                    <Card>
                        <Row>
                            <Col xs={12} className={'d-flex align-items-center justify-content-between mb-3'}>
                                ...
                                <span
                                    className={`label label-inline label-sm label-light-${
                                        presale?.state === 1 ? 'primary' : presale?.state === 3 ? 'danger' : presale?.state === 2 ? 'success' : 'warning'
                                    }`}
                                >
                                    {presale?.state === 1 ? 'Live' : presale?.state === 3 ? 'Failed' : presale?.state === 2 ? 'Success' : 'Upcoming'}
                                </span>
                            </Col>
                            <Col xs={12} className="d-flex align-items-center">
                                ...
                                <div className="d-flex align-items-stretch flex-column">
                                    ...
                                    <div className="d-flex align-items-center mt-2 flex-wrap">
                                        <ExternalLink className={'mr-2'} href={getEtherscanLink(chainId, selectedToken?.address, 'token')}>
                                        ...
                                        <ExternalLink className={'mr-2'} onClick={copyToClipboard.bind(this, selectedToken?.address)}>
                                            <LinkText>{shortenAddress(selectedToken?.address || "")}</LinkText>
                                            <SVG
                                                src={
                                                    require("../../assets/images/account/copy.svg")
                                                        .default
                                                }
                                                width={16}
                                                height={16}
                            ...
                            <Col xs={12} className={'d-flex align-items-center justify-content-around mt-4 mb-3'}>
                                <DetailsWrapper>
                                    <DetailsContainer>
                                        <CircleBar fill={theme.primary} width={40} height={40} percent={Number(liquidityPercent)}/>
                                    </DetailsContainer>
                                ...
                                <DetailsWrapper>
                                    <DetailsContainer>
                                        <CircleBar fill={theme.primary} width={40} height={40} percent={filledPercent}/>
                                    </DetailsContainer>
                                    <DetailsTitle>{filledPercent && !isNaN(filledPercent) ? `${filledPercent}%` : '-'}</DetailsTitle>
                                            ...
                                            <Tab.Content className={"bg-transparent"}>
                                                    ...
                                                    {!account ? (
                                                        ...
                                                        <>
                                                            {presale?.state < 2 && !enoughBalance && (
                                                                <RowCard>
                                                                    <RowTitle className={'text-center'}>Presale Requirements</RowTitle>
                                                                    <RowDesc className={'text-center'}>To participate in presale you need to hold at least the specified amount of the following token.</RowDesc>
                                                                    <div className="d-flex flex-column">
                                                                        <TokenWrapper>
                                                                            <CurrencyLogo currency={BalanceToken} size={'24px'} style={{ marginRight: 16 }}/>
                                                                            <StyledTokenName className={'mt-0'}>8 {BalanceToken.symbol}</StyledTokenName>
                                                                        </TokenWrapper>
                                                                    </div>
                                                                </RowCard>
                                                            )}
                                                            {presale?.state === 2 && depositedToken?.baseDeposited.isGreaterThan(0) && !presale?.status?.lpGenerationComplete && (
                                                                <RowCard>
                                                                    <Row>
                                                                        <Col xs={12} className={'d-flex align-items-center justify-content-center flex-column'}>
                                                                            <RowTitle className={'text-center mb-2'}>Add Liquidity and Lock in Liquidity pool</RowTitle>
                                                                            <DetailsLabel className={'text-center mb-4'}>
                                                                                By adding the liquidity, the withdrawal option will be enabled.
                                                                            </DetailsLabel>
                                                                        </Col>
                                                            ...
                                                            {presale?.state === 2 && depositedToken?.baseDeposited.isGreaterThan(0) && presale?.status?.lpGenerationComplete && (
                                                                <RowCard>
                                                                    <Row>
                                                                        <Col xs={12} className={'d-flex align-items-center justify-content-center flex-column'}>
                                                                            <RowTitle className={'text-center mb-2'}>Withdraw your {selectedToken?.symbol}</RowTitle>
                                                                            <DetailsLabel className={'text-center mb-4'}>
                                                                                You will receive: {depositedToken?.receiveEstimated?.toString()} {selectedToken?.symbol} (approximately)
                                                                            </DetailsLabel>
                                                                        </Col>
                                                                        <Col xs={12} className={'d-flex align-items-center justify-content-center'}>
                                                                            <Button
                                                            ...
                                                            {presale?.state === 3 && depositedToken?.baseDeposited.isGreaterThan(0) && (
                                                                <RowCard>
                                                                    <Row>
                                                                        <Col xs={12} className={'d-flex align-items-center justify-content-center flex-column'}>
                                                                            <RowTitle className={'text-center mb-2'}>ILO failed, Withdraw your {baseToken?.symbol}</RowTitle>
                                                                            <DetailsLabel className={'text-center mb-4'}>
                                                                                You will receive: {depositedToken?.baseDeposited?.toString()} {baseToken?.symbol} (approximately)
                                                                            </DetailsLabel>
                                                                        </Col>
                                                                        <Col xs={12} className={'d-flex align-items-center justify-content-center'}>
                                                                            <Button
                                                            ...
                                                            {presale?.state === 3 && isOwner && (
                                                                <RowCard>
                                                                    <Row>
                                                                        <Col xs={12} className={'d-flex align-items-center justify-content-center flex-column'}>
                                                                            <RowTitle className={'text-center mb-2'}>ILO failed, Withdraw your {baseToken?.symbol} & initial Liquidity</RowTitle>
                                                                        </Col>
                                                                        <Col xs={12} className={'d-flex align-items-center justify-content-center'}>
                                                                            <Button
                                                                                className={'flex-grow-1 ml-1 d-flex align-items-center justify-content-center'}
                                                                    ...
                                                                    <Row>
                                                                        <Col xs={12}>
                                                                            <div className="d-flex align-items-center mb-3">
                                                                                <DetailsContainer className={'mb-0 mr-3'}>
                                                                                    <CircleBar fill={theme.primary} width={40} height={40} percent={100}/>
                                                                                </DetailsContainer>
                                                                                <div className="d-flex flex-column">
                                                                                    <DetailsLabel className={'mb-1'}>Your spent allowance</DetailsLabel>
                                                                                    <DetailsTitleSimple className={'mb-0'}>{maxSpend || "-"} {baseToken?.symbol || "-"}</DetailsTitleSimple>
                                                                                </div>
                                                                            </div>
                                                                        </Col>
                                                                        <Col xs={12}>
                                                                            <RowTitle className={'text-center'}>Spend how much {baseToken?.symbol || "-"}?</RowTitle>
                                                                        </Col>
                                                                        <Col xs={12}>
                                                                            <CurrencyInputPanel
                                                                                ...
                                                                                value={investAmount}
                                                                                showMaxButton={true}
                                                                                currency={baseToken?.symbol === "WETH" || baseToken?.symbol === "ETH" ? ETHER : baseToken}
                                                                        ...
                                                                        <Col xs={12} className={'d-flex align-items-center justify-content-between px-4'}>
                                                                            ...
                                                                            <DetailsTitleSimple style={{ fontSize: 14, marginBottom: 0 }}>{(investAmount * presalePrice).toFixed(4)} {selectedToken?.symbol || "-"}</DetailsTitleSimple>
                                                                        </Col>
                                                                        <Col xs={12}>
                                                                            <Description className={'text-center mt-5'}>
                                                                                Please be aware if the token you are purchasing has deflationary mechanisms such as burn on transfer you may receive less than the amount stated. <br/>
                                                ...
                                                <Tab.Pane eventKey="info">
                                                    <RowCard>
                                                        <Row>
                                                            <Col xs={12}>
                                                                <RowTitle className={'text-center'}>{selectedToken?.symbol || "-"} Tokenomics</RowTitle>
                                                                <RowDesc className={'text-center'}>Total supply: {transformedTotalSupply} {selectedToken?.symbol || "-"}</RowDesc>

                                                            </Col>
                                                            <Col xs={12} className={'mt-4 mb-3'}>
                                                                <RowTitle className={'text-center'}>Presale info</RowTitle>
                                                                <RowDesc className={'mb-1'}>Start block: {presale?.startBlock?.toString() || "-"}</RowDesc>
                                                                <RowDesc className={'mb-1'}>End block: {presale?.endBlock?.toString() || "-"}</RowDesc>
                                                            </Col>
                                                            <Col xs={12}>
                                                                <ListGroup className={'bg-transparent'}>
                                                                    <ListItem>
                                                                        ...
                                                                        <div className="d-flex flex-column justify-content-between">
                                                                            <DetailsTitleSimple style={{ fontSize: 16 }}>{liquidityPercent ? `${liquidityPercent}%` : '-'} {baseToken?.symbol} raised liquidity lock</DetailsTitleSimple>
                                                                            <DetailsLabel>{lockDurationMap?.[lockDuration] || '-'} lock duration</DetailsLabel>
                                                                        </div>
                                                                    </ListItem>
                                                                    <ListItem>
                                                                        <Users size={32} style={{ marginRight: 16 }}/>
                                                                        <div className="d-flex flex-column justify-content-between">
                                                                            <DetailsTitleSimple style={{ fontSize: 16 }}>{presale?.status?.whitelistOnly ? 'Private' : 'Public'}</DetailsTitleSimple>
                                                                            <DetailsLabel>{presale?.status?.whitelistOnly ? 'Whitelisted only' : 'Open to everyone'}</DetailsLabel>
                                                                        </div>
                                                                    </ListItem>
                                                                    <ListItem>
                                                                        <Info size={32} style={{ marginRight: 16 }}/>
                                                                        <div className="d-flex flex-column justify-content-between">
                                                                            <DetailsTitleSimple style={{ fontSize: 16 }}>{softCap || '-'} {baseToken?.symbol || "-"}</DetailsTitleSimple>
                                                                    ...
                                                                    <ListItem>
                                                                        <Info size={32} style={{ marginRight: 16 }}/>
                                                                        <div className="d-flex flex-column justify-content-between">
                                                                            <DetailsTitleSimple style={{ fontSize: 16 }}>{hardCap || '-'} {baseToken?.symbol || "-"}</DetailsTitleSimple>
                                                                    ...
                                                                    <ListItem>
                                                                        <Info size={32} style={{ marginRight: 16 }}/>
                                                                        <div className="d-flex flex-column justify-content-between">
                                                                            <DetailsTitleSimple style={{ fontSize: 16 }}>{maxSpend || "-"} {baseToken?.symbol || "-"}</DetailsTitleSimple>
                                                                            <DetailsLabel>Max spend per account</DetailsLabel>
                                                                        </div>
                                                                    </ListItem>
                                                                    <ListItem>
                                                                        <Info size={32} style={{ marginRight: 16 }}/>
                                                                        <div className="d-flex flex-column justify-content-between">
                                                                            <DetailsTitleSimple style={{ fontSize: 16 }}>{presalePrice} per {baseToken?.symbol || "-"}</DetailsTitleSimple>
                                                                    ...
                                                                    <ListItem>
                                                                        <Info size={32} style={{ marginRight: 16 }}/>
                                                                        <div className="d-flex flex-column justify-content-between">
                                                                            <DetailsTitleSimple style={{ fontSize: 16 }}>{listingPrice} per {baseToken?.symbol || "-"}</DetailsTitleSimple>
