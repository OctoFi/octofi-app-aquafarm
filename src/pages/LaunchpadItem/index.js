...
const LaunchpadItem = props => {
    ...
    const depositedToken = useMemo(() => {
        ...
        if(buyState?.baseDeposited && listingPrice) {
            ...
            console.log(buyState?.baseDeposited?.toString(), baseDeposited?.toString());
            const receiveEstimated = baseDeposited.times(listingPrice);
    ...
    const purchaseHandler = () => {
        ...
        const args = [
            baseToken?.symbol === 'WETH' ? new BigNumber(0).toString() : new BigNumber(investAmount).times(10 ** baseToken?.decimals).toString()
        ]


        contract.estimateGas.userDeposit(...args, { value: baseToken?.symbol === 'WETH' ? new BigNumber(investAmount).times(10 ** baseToken?.decimals).toString() : 0 })
            .then(res => {
                contract.userDeposit(...args, {
                    gasLimit: calculateGasMargin(res),
                    value: baseToken?.symbol === 'WETH' ? new BigNumber(investAmount).times(10 ** baseToken?.decimals).toString() : 0
                }).then(() => {
                    setInvestAmount('');
                                                                ...
                                                                <RowCard>
                                                                    ...
                                                                    <div className="d-flex flex-column">
                                                                        <TokenWrapper>
                                                                            ...
                                                                            <StyledTokenName className={'mt-0'}>1 {BalanceToken.symbol}</StyledTokenName>
                                                                        </TokenWrapper>
                                                                    </div>
                                                                </RowCard>
                                                            )}
