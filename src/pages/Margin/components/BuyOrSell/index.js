    ...
    return (
        <Card>
            <TabsContainer className="d-flex align-items-center justify-content-between">
                <TabWrapper>
                    <Tab variant={activeSide === OrderSide.Buy ? 'secondary-light' : 'light-secondary-light'} onClick={setActiveSide.bind(this, OrderSide.Buy)}>
                        {t('orderSide.buy')}
                    </Tab>
                </TabWrapper>
                <TabWrapper>
                    <Tab variant={activeSide === OrderSide.Sell ? 'secondary-light' : 'light-secondary-light'} onClick={setActiveSide.bind(this, OrderSide.Sell)}>
                        {t('orderSide.sell')}
                    </Tab>
                </TabWrapper>
            </TabsContainer>
            <Content>
                <MarketType>
                    {selectedMarketStats?.supportedOrderTypes?.map((tab, index) => {
                        return (
                            <>
                                <MarketButton className="btn btn-link px-0" disabled={tab === activeOrderType} onClick={() => {
                                    setActiveOrderType(tab)
                                }}>{orderTypes[tab]}</MarketButton>
                                {index < selectedMarketStats?.supportedOrderTypes?.length - 1 && (<span className={'px-2'}>/</span>)}
                            </>
                        )
                    })}
                </MarketType>
                ...
                <div className="d-flex flex-column align-items-xl-center align-items-stretch justify-content-center mt-auto">
                    ...
                    >
                        {!account ? t('wallet.connect') : `${activeSide === OrderSide.Buy  ? t('orderSide.buy') : t('orderSide.sell')} ${selectedMarketStats?.baseAsset || ''}`}
                    </Button>
                </div>
            </Content>
        </Card>
