...
const orderToRow = (order, index, cancelOrder, market) => {
    ...
    const filled = order.confirmedAmount ? tokenAmountInUnits(new BigNumber(order.confirmedAmount), baseDecimals, 3) : null;
    if(order.status) {
    ...
    if(!accessToPrivate) {
        content = (
            <div className="w-100 h-100 d-flex align-items-center justify-content-center flex-column">
                ...
                <Button variant={'secondary'} onClick={signer}>{t('confirm')}</Button>
            </div>
        )
