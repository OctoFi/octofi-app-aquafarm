...
const fillToRow = (fill, index) => {
    ...
    return (
        <TR key={index}>
            ...
            <TD>
                {moment(fill.createdAt).format('HH:mm:ss')}
            </TD>
        </TR>
    );
};
    ...
    if(!selectedMarket || tradesStats === ServerState.NotLoaded) {
        content = (
            <div className="w-100 h-100 d-flex align-items-center justify-content-center">
                <Loading width={40} height={40} id={`margin-trading-history-${props.isMobile ? 'mobile' : 'desktop'}`} active/>
            </div>
        )
        ...
        content = (
            <TableContainer className="d-flex flex-column">
                <CustomHead>
                    <TH>{t('size')}</TH>
                    <TH >{t('price')} ({selectedMarket.quoteAsset})</TH>
                    <TH>{t('time')}</TH>
                </CustomHead>
                <TBody>
                    {trades.map((t, i) => fillToRow(t, i, selectedMarket))}
                </TBody>
            </TableContainer>
        )
    }
