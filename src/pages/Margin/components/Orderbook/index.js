...
class OrderToRow extends Component {
    render() {
        ...
        const mySize = mySizeOrders ? mySizeOrders?.reduce((sumSize, mySizeItem) => {
        ...
        return (
            <InnerTR
                onClick={() => this._setOrderPriceSelected(price)}
            >
                <TD>{size}</TD>
...
class Orderbook extends Component {
    ...
    getSpreadPercent = (spread = undefined) => {
        ...
        const lowestSellPrice = Number(asks?.[0]?.price) * (10 ** Math.abs(selectedMarket?.baseAssetDecimals - selectedMarket?.quoteAssetDecimals));
        ...
        if(!selectedMarket || orderbookState === ServerState.NotLoaded){
            content = (
                <div className="w-100 h-100 d-flex align-items-center justify-content-center">
                    <Loading width={40} height={40} id={`margin-orderbooks-${this.props.isMobile ? 'mobile' : 'desktop'}`} active/>
                </div>
            )
            ...
            if(Array.isArray(myOrders)) {
                mySizeSellArray = myOrders ? myOrders?.filter((order) => {
                    return order?.side === 'sell' && order?.status === 'pending';
                }) : [];
                mySizeBuyArray = myOrders ? myOrders?.filter((order) => {
            ...
            content = (
                <>
                    ...
                    <TableRow>
                        <CustomHead>
                            <TH>{t('size')}</TH>
                            <TH>{t('price')} ({formatTokenSymbol(selectedMarket?.quoteAsset)})</TH>
                            <TH>{t('mySize')}</TH>
                        </CustomHead>
                        <TBody>

                            {((bookOptionState === BookOptionType.BuySellBook ||
                                bookOptionState === BookOptionType.SellBook)) && asks
                                .map((order, index) => (
                                    <OrderToRowContainer
                                        key={index}
                                        order={order}
