...
class History extends React.Component {
	...
	fetchTransactions = async () => {
		...
		let txnRes = await axios.get(
			`https://api.etherscan.io/api?module=account&action=txlist&address=${this.props.web3.account}&startblock=0&endblock=${lastBlockNumber}&page=1&offset=${PAGE_SIZE}&sort=desc&apikey=KM1S3UP6BGICACR5X2IE5E3ZIADV33DKA1`
		);
		let erc20Res = await axios.get(
			`https://api.etherscan.io/api?module=account&action=tokentx&address=${this.props.web3.account}&startblock=0&endblock=${lastBlockNumber}&page=1&offset=${PAGE_SIZE}&sort=desc&apikey=KM1S3UP6BGICACR5X2IE5E3ZIADV33DKA1`
		);
		...
		this.setState((prevState) => {
			...
			for (let i in result) {
				...
				if (item > -1) {
					sections[item].content.push(
						...transactions.map((t) => {
							const txn = result[i][t];
							return <Collapse txn={txn} />;
						})
					);
				} else {
					sections.push({
						...
						<CustomCard
							...
							header={
								<Header className={"d-none d-md-flex"}>
									...
									<CSVLink
										className={"btn btn-light-primary py-3"}
										data={this.state.transactions}
										filename={`${this.props.web3.account}_${this.state.blockNumber}__defi_dashboard.csv`}
									>
										{t("download", { file: "CSV" })}
									</CSVLink>
								</Header>
