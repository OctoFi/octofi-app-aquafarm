			...
			if (res) {
				for (let obj of hashArr) {
					if (res.hash === obj.hash) {
						setMintBTCErrorTip(`No new deposit transaction found. Please send ${formatCoin(inputSymbol)} to deposit address first.`)
						MintModelView()
						return
					}
				}
	...
	function walletTip () {
		...
		if (node) {
			...
			return (
				...
				<dd><i></i>💀 {`ONLY the deposits from your ${coin} wallet ${account} will be credited!!!`}</dd>
			)
		}
	}
			...
			return (
				<Col xs={12}>
					<AlertContainer className={'mt-4'}>
						<AlertText>
							You need have <strong>{coin} and 0.02 {config.bridgeAll[node].symbol} gas fee</strong> on your same eth wallet first: {shortenAddress(account)}
						</AlertText>
					</AlertContainer>
				</Col>
			)
					...
					<Modal.Body>
						<MintDiv>
							{inputValueFormatted ? (
								<>
									<MintList>
										<MintListLabel>Deposit {inputSymbol && formatCoin(inputSymbol)} amount:</MintListLabel>
							...
							<MintList>
								<MintListLabel>Deposit {inputSymbol && formatCoin(inputSymbol)} address:</MintListLabel>
						...
						<Col xs={12}>
							<Card>
								<MintTip onClick={MintInfoModelView}>
									<Flex className='pd0'>
										<Flex className='pd0'><TokenLogo size={'32px'} address={inputSymbol ? 'BTC' : formatCoin(inputSymbol)} /></Flex>
										<MintListVal className="txt"><Flex className='pd0'>Waiting for deposit</Flex></MintListVal>
									</Flex>
								</MintTip>
							</Card>
						</Col>
						...
						<Row>
							<Col xs={12}>
								<BridgeInputPanel
									...
									onUserInput={inputValue => {
										...
										let inputVal = iValue && Number(iValue) ? new BigNumber(ethers.utils.parseUnits(iValue.toString(), inputDecimals).toString()) : new BigNumber(0)
										let _fee = inputVal.times(ethers.utils.parseUnits(dFee.toString(), 18).toString()).dividedBy(new BigNumber(10).pow(new BigNumber(18)).toString());
										let _minFee = new BigNumber(ethers.utils.parseUnits(dMinFee.toString(), inputDecimals).toString());
										let _maxFee = new BigNumber(ethers.utils.parseUnits(dMaxFee.toString(), inputDecimals).toString());
										if (bridgeType && bridgeType === 'redeem') {
											_fee = inputVal.times(ethers.utils.parseUnits(fee.toString(), 18).toString()).dividedBy(new BigNumber(10).pow(new BigNumber(18)).toString());
											_minFee = new BigNumber(ethers.utils.parseUnits(minFee.toString(), inputDecimals).toString());
											_maxFee = new BigNumber(ethers.utils.parseUnits(maxFee.toString(), inputDecimals).toString());
										}
										if (_fee.isZero()) {
											// inputVal = inputVal
										} else {
							...
							{bridgeType && bridgeType === 'redeem'  ? (
								...
								<Col xs={12} className={'pt-4'}>
									<AddressInputPanel
										...
										withoutMargin={true}
										label={'Deposit ' + (inputSymbol ? formatCoin(inputSymbol) : inputSymbol)  + ' Address'}
									/>
								</Col>
							) : null}
...

							{
								isLimitAction ? '' : (
									<Col xs={12} className={'pt-4'}>
										<AlertContainer>
											<AlertText>The liquidity of the cross chain bridge is insufficient, please try again later. Balance: {thousandBit(limitAmount,2)} {inputSymbol}</AlertText>
										</AlertContainer>
									</Col>
								)
							}
