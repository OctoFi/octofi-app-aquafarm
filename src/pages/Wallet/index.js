...
const Wallet = props => {
                ...
                return (
                    <div className={'d-flex align-items-stretch justify-content-center flex-column flex-lg-row align-items-lg-center justify-content-lg-start w-100'}>
                        {isLoading ? (
                            ...
                            <>
                                <StyledLink to={`/swap/uni?outputCurrency=${row.metadata.symbol === 'ETH' ? 'ETH' : row.metadata.address}`}>
                                    <TradeButton>
                                        {t('buttons.buy')}
                                    </TradeButton>
                                </StyledLink>
                                ...
                                <StyledLink>
                                    <TradeButton variant={'warning'} onClick={unlockHandler.bind(this, row.metadata)} disabled={row.metadata.symbol === 'ETH'}>
                                        {t('buttons.unlock')}
                                    </TradeButton>
                                </StyledLink>

                                <StyledLink to={`/swap/uni?inputCurrency=${row.metadata.symbol === 'ETH' ? 'ETH' : row.metadata.address}`}>
                                    <TradeButton variant={'secondary2'}>
                                        {t('buttons.sell')}
                                    </TradeButton>
                                </StyledLink>
                                {value <= 0.001 ? (
                                    <StyledLink to={`/swap/uni?inputCurrency=${row.metadata.symbol === 'ETH' ? 'ETH' : row.metadata.address}&outputCurrency=0x7240aC91f01233BaAf8b064248E80feaA5912BA3`} disabled>
    ...
    return (
        <Page title={t('wallet.title')} morePadding>
            <UnlockModal done={done} show={showUnlockModal} unlocking={unlocking} onDismiss={() => {
                setDone(false);
                setUnlocking(false);
                setShowUnlockModal(false);
            }}/>
            ...
            <ResponsiveCard style={{ marginTop: -30 }}>
                <Tab.Container defaultActiveKey={'tokens'}>
                    <Row>
                        <HeaderCol xs={12} className={'d-flex flex-column-reverse flex-lg-row align-items-stretch align-items-lg-center justify-content-between'}>
                            ...
                            <CustomInputGroup className={'w-auto mb-lg-0'} bg={'darker'}>
                                ...
                                <FormControl id="walletSearch" placeholder={t('search')} onChange={e => setQuery(e.target.value)}/>
                            </CustomInputGroup>
                        </HeaderCol>
                        <Col xs={12}>
