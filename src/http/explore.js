...
export default class ExploreApi {
    constructor() {
        this.requestBaseURLs = {
            ...
            topTokens: 'https://api.coingecko.com/api/v3/coins/markets?vs_currency=USD&category=decentralized_finance_defi&price_change_percentage=1h,24h,7d,30d,60d,200d,1y'
        }
    }
    ...
    fetchTokens() {
        return axios.get(`${this.requestBaseURLs.tokens}/markets?vs_currency=USD&price_change_percentage=1h,24h,7d,30d,60d,200d,1y`)
            .then(response => {
    ...
    fetchTokenSets() {
        return axios.get(this.requestBaseURLs.tokenSets)
            .then(response => {
    ...
    fetchPools() {
        ...
        return axios.get(`${this.requestBaseURLs.pools}/v1/exchanges?${query}`)
            .then(response => {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                for(let i in trending.data.coins) {
                    const res = await axios.get(`${this.requestBaseURLs.tokens}/${trending.data.coins[i].item.id}?community_data=false&developer_data=false`)
