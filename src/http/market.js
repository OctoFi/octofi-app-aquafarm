...
export default class MarketApi {
    ...
    async fetchSinglePrice(payload) {
        ...
        const links = {
            ...
            nomics: `https://api.nomics.com/v1/currencies/sparkline?key=${process.env.REACT_APP_NOMICS_API_KEY}&ids=${coin.symbol}&start=${today}`,
        ...
        try {
            nomics = await axios.get(links.nomics);
            result.nomics = Number(nomics.data[0].prices[nomics.data[0].prices.length - 1]);
        } catch(e) {
            result.nomics = 0;
        }
        try {
