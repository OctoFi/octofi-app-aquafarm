...
const Collapse = (props) => {
    ...
    return (
        <Wrapper show={show}>
            <Header className="d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-between" ref={header} onClick={showCollapse} show={show} >
                ...
                <HeaderSection className="d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-start " type={'tokens'}>
                            ...
                            <div className="d-flex justify-content-center flex-column" style={{ marginLeft: 5 }}>
                                <Value>{
                                    tokens.from.length !== 1 ? (
                                        <Desc><Value>{tokens.from.length}</Value> {t('tokens.assets')}</Desc>
                                    ) : `-${tokens.from[0].tokenDecimal ? (Number(tokens.from[0].value) / 10 ** Number(tokens.from[0].tokenDecimal || 18)).toFixed(4) : (Number(tokens.from[0].value) / 10 ** 18).toFixed(4)} ${tokens.from[0].tokenSymbol || 'ETH'}`
                                }</Value>
                                {tokens.from.length === 1 && (
                                    <Desc>{tokens.from[0].contractAddress ? hashText(tokens.from[0].contractAddress) : 'Ethereum'}</Desc>
                                )}
                    ...
                    {tokens.to.length > 0 && (
                        <TradeSide className="d-flex align-items-center">
                            ...
                            <div className="d-flex justify-content-center flex-column"  style={{ marginLeft: 5 }}>
                                <Value color={'primary'}>{
                                    tokens.to.length !== 1 ? (
                                        <Desc><Value>{tokens.to.length}</Value> {t('tokens.assets')}</Desc>
                                    ) : `+${tokens.to[0].tokenDecimal ? (Number(tokens.to[0].value) / 10 ** Number(tokens.to[0].tokenDecimal || 18)).toFixed(4) : (Number(tokens.to[0].value) / 10 ** 18).toFixed(4)} ${tokens.to[0].tokenSymbol || 'ETH'}`
                                }</Value>
                                {tokens.to.length === 1 && (
                                    <Desc>{tokens.to[0].contractAddress ? hashText(tokens.to[0].contractAddress) : 'Ethereum'}</Desc>
                                )}
                ...
                <HeaderSection className="d-flex flex-column flex-lg-row align-items-stretch align-items-lg-center justify-content-center justify-content-lg-end" type={'application'}>
                    <div className="d-flex flex-column justify-content-center align-items-stretch align-items-lg-start">
                        ...
                        <Application className="label label-inline label-light-primary label-lg">{tokens.from.length > 0 ? hashText(tokens.from[0].to || tokens.from[0].contractAddress) : tokens.to.length > 0 ? hashText(tokens.to[0].from || tokens.to[0].contractAddress) : ''}</Application>
                                    ...
                                    {tokens.from.map(token => {
                                        ...
                                        return (
                                            <BodyToken>
                                                ...
                                                <div className="d-flex justify-content-center flex-column" style={{ marginLeft: 30 }}>
                                                    <Value>{`-${token.contractAddress ? (Number(token.value) / 10 ** Number(token.tokenDecimal || 18)).toFixed(4) : (Number(token.value) / 10 ** 18).toFixed(4)} ${token.tokenSymbol || 'ETH'}`}</Value>
                                                    <Desc>{token.contractAddress ? hashText(token.contractAddress) : 'Ethereum'}</Desc>
                                                </div>
                                            </BodyToken>
                                        )
                                    })}
