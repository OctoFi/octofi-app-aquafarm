                        ...
                        <TradeSide className="d-flex align-items-center">
                            {tokens.from.map(token => {
                                if(token.contractAddress) {
                                    const currency = new Token(chainId, token.contractAddress, Number(token.tokenDecimal), token.tokenSymbol, token.tokenName);
                                    return (
                                        <div className={'mr-2'}>
                            ...
                            <div className="d-flex justify-content-center flex-column ml-1">
                                <span className="font-size-lg font-weight-bold">{
                                    tokens.from.length !== 1 ? `${tokens.from.length} Assets` : `-${tokens.from[0].contractAddress ? (Number(tokens.from[0].value) / 10 ** Number(tokens.from[0].tokenDecimal)).toFixed(4) : (Number(tokens.from[0].value) / 10 ** 18).toFixed(4)} ${tokens.from[0].tokenSymbol || 'ETH'}`
                                }</span>
                        ...
                        <TradeSide className="d-flex align-items-center">
                            ...
                            <div className="d-flex justify-content-center flex-column ml-3">
                                <span className="font-size-lg text-success font-weight-bold">{
                                    tokens.to.length !== 1 ? `${tokens.to.length} Assets` : `+${tokens.to[0].contractAddress ? (Number(tokens.to[0].value) / 10 ** Number(tokens.to[0].tokenDecimal)).toFixed(4) : (Number(tokens.to[0].value) / 10 ** 18).toFixed(4)} ${tokens.to[0].tokenSymbol || 'ETH'}`
                                }</span>
                                {tokens.to.length === 1 && (
                    ...
                    <>
                        ...
                        <Body className="d-flex align-items-center pb-4">
                            <div className="d-flex flex-column justify-content-center">
                                {tokens.from.map(token => {
                                        const currency = token.contractAddress ? new Token(chainId, token.contractAddress, Number(token.tokenDecimal), token.tokenSymbol, token.tokenName) : ETHER;
                                        return (
                                            <div className="d-flex align-items-center py-2">
                                                <CurrencyLogo currency={currency} size={'36px'}/>
                                                <div className="d-flex justify-content-center flex-column ml-3">
                                                    <span className="font-size-lg font-weight-bold">{`-${token.contractAddress ? (Number(token.value) / 10 ** Number(token.tokenDecimal)).toFixed(4) : (Number(token.value) / 10 ** 18).toFixed(4)} ${token.tokenSymbol || 'ETH'}`}</span>
                                ...
                                {tokens.to.map(token => {
                                    ...
                                    return (
                                        <div className="d-flex align-items-center py-2">
                                            <CurrencyLogo currency={currency} size={'36px'}/>
                                            <div className="d-flex justify-content-center flex-column ml-3">
                                                <span className="font-size-lg text-success font-weight-bold">{`+${token.contractAddress ? (Number(token.value) / 10 ** Number(token.tokenDecimal)).toFixed(4) : (Number(token.value) / 10 ** 18).toFixed(4)} ${token.tokenSymbol || 'ETH'}`}</span>
                                                <span className={'text-muted'}>{token.contractAddress ? hashText(token.contractAddress) : 'Ethereum'}</span>
                                            </div>
                                        </div>
                                    )
