                        ...
                        <Col xs={12}>
                            {!account ? (
                                ...
                                <Button
                                    ...
                                    variant={!parsedAmounts[Field.CURRENCY_A] || Number(maxAmounts[Field.CURRENCY_A]?.toSignificant(6)) < Number(parsedAmounts[Field.CURRENCY_A]?.toSignificant(6))? 'light' : 'primary'}
                                    className={`${!!parsedAmounts[Field.CURRENCY_A] || Number(maxAmounts[Field.CURRENCY_A]?.toSignificant(6)) >= Number(parsedAmounts[Field.CURRENCY_A]?.toSignificant(6))? 'light-primary' : ''} py-6`}
                                >
                                    <span className="font-weight-bold font-size-lg">
                                        {!parsedAmounts[Field.CURRENCY_A] ? 'Please Enter Amount' :
                                           Number(maxAmounts[Field.CURRENCY_A]?.toSignificant(6)) < Number(parsedAmounts[Field.CURRENCY_A]?.toSignificant(6)) ? 'Insufficient balance' : 'Supply'}
                                    </span>
