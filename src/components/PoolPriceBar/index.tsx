...
export function PoolPriceBar({
  ...
  return (
    <div className={'d-flex flex-column'}>
      <Row>
        <Col order={0}>
          <HeaderText fontWeight={500} color={theme.text1} >{price?.toSignificant(6) ?? '-'}</HeaderText>
        </Col>
        <Col order={0}>
          <HeaderText fontWeight={500} color={theme.text1} >{price?.invert()?.toSignificant(6) ?? '-'}</HeaderText>
        </Col>
        <Col order={0}>
