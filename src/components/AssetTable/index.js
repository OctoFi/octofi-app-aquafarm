...
const AssetTable = (props) => {
	...
	return (
		<div className={"table-responsive"}>
			<table
				className={`table currency__table table-borderless mb-0 table-head-custom ${
					props.size ? `currency__table--${props.size}` : ""
				}`}
			>
				<thead>
					<tr>
					...
					{data.map((row) => {
						if (row.underlying.length > 0) {
							return row.underlying.map((uRow) => {
								const currency = uRow.metadata.address.toLowerCase() === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' ? ETHER : new Token(
									chainId,
							...
							const currency = row.base.metadata.address.toLowerCase() === '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' ? ETHER : new Token(
								chainId,
