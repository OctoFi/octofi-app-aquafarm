		...
		try {
			...
			for (let i = startBlock; i < endBlock; i = i + step) {
				...
				if (!this.state.series[0].data.find((x) => (x.hasOwnProperty("block") ? x.block === i : false))) {
					...
					let balancePromise = this.promisify((cb) => this.web3Provider.eth.getBalance(address, i, cb));
			...
			for (let i = 0; i < results.length; i = i + 3) {
				balances.push({
					block: results[i],
					balance: parseFloat(this.web3Provider.utils.fromWei(results[i + 1], 'ether')),
