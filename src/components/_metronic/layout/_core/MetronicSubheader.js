...
export function getBreadcrumbsAndTitle(menuId, pathName) {
  ...
  const customElement = document.querySelector('[data-breadcrumbs]');
  if(customElement) {
    result.title = customElement.dataset.title || '';
    result.breadcrumbs = JSON.parse(customElement.dataset.breadcrumbs);
