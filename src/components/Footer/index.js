        ...
        <Container className="footer" desktop={DesktopImage} mobile={MobileImage}>
            ...
            <Content className="footer__content w-100 d-flex flex-column flex-lg-row">
                ...
                <FooterAbout className={'d-flex flex-column footer__about'}>
                    ...
                    <div className="d-none d-lg-flex flex-column">
                        ...
                        <CopyRight className="footer__copyright">
                            <Trans i18nKey={'app.copyright'} values={{ company: "Stealth"}}>
                                <a href="#" className={'link'}>Decentralized Finance Tentacles</a>
                            </Trans>
                        </CopyRight>
                    </div>
                </FooterAbout>
                        ...
                        {Object.keys(routes).map(key => {
                            ...
                            return (
                                <Links className="footer__links" key={key}>
                                    <li className={'footer__item'}><StyledLink to={r.hasOwnProperty('path') ? r.path : '#'} className={'footer__link footer__link--primary'}>{t(`menu.${r.title}`)}</StyledLink></li>
                                    {r.hasOwnProperty('routes') && Object.values(r.routes).map((item, index) => {
                                        return (
                                            <li className={'footer__item'}  key={`${key}-${index}`}>
                                                <StyledLink to={item.path} className={'footer__link'}>{t(`menu.${item.title}`)}</StyledLink>
                                            </li>
                                        )
                                    })}
                                </Links>
                ...
                <MobileSocials className="d-flex d-lg-none flex-column align-items-center justify-content-center footer__mobile-socials">
                    <Socials/>
                    <CopyRight className="footer__copyright">
                        <Trans i18nKey={'app.copyright'} values={{ company: "Stealthex"}}>
                            <a href="#" className={'link'}>Decentralized Finance Tentacles</a>
                        </Trans>
                    </CopyRight>
                </MobileSocials>
