...
import { ConfirmationModalContent, ConfirmationPendingContent, TransactionSubmittedContent } from '../TransactionConfirmationModal'
...
export default function({
    ...
    async function onRemove() {
        ...
        // @ts-ignore
        const selectedTokenAddress = (!token?.symbol || (token?.symbol && token?.symbol.toUpperCase() === 'ETH')) ? '0x0000000000000000000000000000000000000000' : token?.address;
        
        setButtonState(BUTTON_STATES.create_tx);

        let method: (...args: any) => Promise<TransactionResponse>,
            args: Array<string | string[] | number>
        let estimate = type === 'Balancer' ? router.estimateGas.EasyZapOut : selectedOption === 'pair' ? router.estimateGas.ZapOut2PairToken : router.estimateGas.ZapOut
        method = type === 'Balancer' ? router.EasyZapOut : selectedOption === 'pair' ? router.ZapOut2PairToken : router.ZapOut
        args = []
        if(type && type.toLowerCase() === 'curve') {
            args = [
                account,
    ...
    const selectCurrencyHandler = useCallback((e) => {
        setToken(e);
    }, [setToken])
    ...
    const modalBottom = () => {
        return (
            <>
                ...
                <Button variant={'primary'} className={'py-3 font-weight-bolder font-size-lg'} style={{ margin: '20px 0 0 0', maxHeight: 56 }} onClick={onRemove}>
                    {buttonState}
                </Button>
            </>
        )
                ...
                {!account ? (
                    ...
                    <Row>
                        <Col xs={12}>
                            <AccountState type={'danger'} className={'bg-light-danger'}>
                                <SVG src={require('../../assets/images/account/wallet.svg').default} width={36} height={36}/>
                                <AccountStateContent>
                                    <AccountStateTitle className={'mb-0'}>{t('wallet.notConnected')}</AccountStateTitle>
                                </AccountStateContent>
                            </AccountState>
                        </Col>
                        <Col xs={12} className={'d-flex align-items-center justify-content-center'} style={{ padding: '80px 0 88px' }}>
                            <GradientButton
                                className={'btn-lg'}
                    ...
                    <Row>
                        <Col xs={12}>
                            <AccountState type={'success'} className={'bg-light-success d-none d-xl-flex'}>
                                ...
                                <AccountStateContent>
                                    ...
                                    <AccountStateDesc>{t('wallet.connectedTo')} <strong>{account}</strong></AccountStateDesc>
                                </AccountStateContent>
                            </AccountState>
                        </Col>
                            ...
                            <LightCard padding={'0'} borderRadius={'18px'} style={{ marginBottom: 20 }}>
                                ...
                                <PlatformControl>
                                    <Row>
                                        {type === 'Uniswap' && (
                                            <Col xs={12}>
                                                <Check type={'radio'} id={`withdraw-pair`} className={'d-flex align-items-center py-2 ml-2'} custom>
                                                    <Check.Input type={'radio'} name={'withdraw'} checked={selectedOption === 'pair'} onChange={() => setSelectedOption('pair')} />
                                                    <Check.Label className={'d-flex flex-column pl-2 wallet-modal__label'}>
                                                        <div className={'font-weight-bold'} style={{ marginTop: '.375rem' }}>{t('pools.inDepositedTokens')}</div>
                                                    </Check.Label>
                                                </Check>
                                                <Check type={'radio'} id={`withdraw-token`} className={'d-flex align-items-center py-2 ml-2'} custom>
                                                    <Check.Input type={'radio'} name={'withdraw'} checked={selectedOption === 'custom'} onChange={() => setSelectedOption('custom')} />
                                                    <Check.Label className={'d-flex flex-column pl-2 wallet-modal__label'}>
                                                        <div className={'font-weight-bold'} style={{ marginTop: '.375rem' }}>{t('pools.inCustomToken')}</div>
                                                    </Check.Label>
                                                </Check>
                                            </Col>
                                        )}
                        ...
                        <Col xs={12} className={'d-flex d-xl-none flex-column'}>
                            <div className={'d-flex flex-column'}>
                                ...
                                <PlatformControl>
                                        {type === 'Uniswap' && (
                                            <div>
                                                <Check type={'radio'} id={`withdraw-pair-mobile`} className={'d-flex align-items-center py-2 ml-2'} custom>
                                                    <Check.Input type={'radio'} name={'withdraw-mobile'} checked={selectedOption === 'pair'} onChange={() => setSelectedOption('pair')} />
                                                    <Check.Label className={'d-flex flex-column pl-2 wallet-modal__label'}>
                                                        <div className={'font-weight-bold'} style={{ marginTop: '.375rem' }}>{t('pools.inDepositedTokens')}</div>
                                                    </Check.Label>
                                                </Check>
                                                <Check type={'radio'} id={`withdraw-token-mobile`} className={'d-flex align-items-center py-2 ml-2'} custom>
                                                    <Check.Input type={'radio'} name={'withdraw-mobile'} checked={selectedOption === 'custom'} onChange={() => setSelectedOption('custom')} />
                                                    <Check.Label className={'d-flex flex-column pl-2 wallet-modal__label'}>
                                                        <div className={'font-weight-bold'} style={{ marginTop: '.375rem' }}>{t('pools.inCustomToken')}</div>
                                                    </Check.Label>
                                                </Check>
                        ...
                        <Col xs={12} className={'gutter-b'}>
                            <LightCard padding={'0'} borderRadius={'18px'} style={{ marginBottom: 20 }}>
                                <PriceTopbar>
                                    {t('pools.selectGasSetting')}
                                </PriceTopbar>
                                <GasPrice gasList={gasPrice} selected={selectedGasPrice}/>
                            </LightCard>
                        </Col>
                        <Col xs={12} className={'d-flex flex-column align-items-stretch align-items-xl-center justify-content-center'}  style={{ paddingTop: 30 }} >
                            {!account ? (
                                <Button
                                    style={{ minWidth: 250 }}
                                    variant={'outline-primary'} disabled className={'py-3 font-size-lg font-weight-bolder'}>Connect to your wallet</Button>
                            ) : (
                                <Button
                                    onClick={() => {
                                    ...
                                    variant={!amount || Number(selectedCurrencyBalance) < Number(amount)? 'outline-primary' : 'primary'}
                                    className={`${!!amount || Number(selectedCurrencyBalance) >= Number(amount) || Number(amount) === 0? 'outline-primary' : ''} py-3`}
                                >
                                    <span className="font-weight-bold font-size-lg">
                                        {!amount || Number(amount) === 0 ? t('exchange.enterAmount') :
                                           Number(selectedCurrencyBalance) < Number(amount) ? t('insufficientBalance') :
                                           approve === ApprovalState.NOT_APPROVED || approve === ApprovalState.UNKNOWN ? t('approve') :
                                           approve === ApprovalState.PENDING ? t('pending') : t('pools.withdraw')}
                                    </span>
