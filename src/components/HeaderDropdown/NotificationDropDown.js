...
const NotificationDropdown = (props) => {
    ...
    const pendingTransactions = useMemo(() => {
        return Object.values(transactions)?.filter(txn => !txn?.hasOwnProperty('confirmedTime'))?.slice(0, 3);
    }, [transactions])

    const confirmedTransactions = useMemo(() => {
        return Object.values(transactions)?.filter(txn => txn?.hasOwnProperty('confirmedTime') && typeof txn.confirmedTime === 'number')?.slice(0, 3);
    }, [transactions])
        ...
        <Wrapper>
            ...
            <DropDown className={'header-dropdown'}>
                <Row>
                    ...
                    <Col xs={12}>
                        <StyledBox>
                            {pendingTransactions && pendingTransactions?.length > 0 ? pendingTransactions.map(txn => {
                                return (
                                    <TxnBox href={txn?.hash ? getEtherscanLink(1, txn?.hash, 'transaction') : '#'} target={'_blank'} rel={'noreferrer noopener'}>
                ...
                <Row>
                    ...
                    <Col xs={12}>
                        <StyledBox style={{ marginBottom: 0 }}>
                            {confirmedTransactions && confirmedTransactions?.length > 0 ? confirmedTransactions.map(txn => {
                                return (
                                    <TxnBox href={txn?.hash ? getEtherscanLink(1, txn?.hash, 'transaction') : '#'} target={'_blank'} rel={'noreferrer noopener'}>
                                        {txn?.hash ? txn?.hash?.slice(0, 6) + '...' + txn?.hash?.slice(-4) : '-'} 🡕
                                    </TxnBox>
