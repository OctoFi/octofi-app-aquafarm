            ...
            <Wrapper open={open}>
                <Content>
                    <Header>
                        ...
                        <span className={'font-size-sm'}>Menu</span>
                    </Header>
                    <LogoContainer>
                        <Logo/>
                    </LogoContainer>
                    {account && (
                        <SideDrawerItem title={'Account'} items={accountRoutes} icon={(<UserIcon size={'20px'} style={{ color: theme.primary}}/>)}/>
                    )}
                    ...
                    {Object.keys(routes).map(key => {
                        ...
                        if(r.hasOwnProperty('path')) {
                            return (
                                <LinkItem to={r.path}>{r.title}</LinkItem>
                            )
                        } else {
                            return (
                                <SideDrawerItem title={r.title} items={r.routes}/>
                            )
                        }
                    })}
                </Content>
