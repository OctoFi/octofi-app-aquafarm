                    ...
                    <LedgerContentWrapper>
                        <Row className={'row-paddingless'}>
                            {LedgerPaths.map((item, index) => {
                                return (
                                    <Col xs={12} lg={4} md={6} key={`hd-path-${index}`}>
                                        <Check type={'radio'} id={`hd-path-${index}`} className={'d-flex align-items-center py-3'} custom>
                            ...
                            <Col xs={12} lg={4} md={6} key={`hd-path-custom`}>
                                <Check type={'radio'} id={`hd-path-custom`} className={'d-flex align-items-center mb-3 pt-3 pt-xl-0'} custom>
        ...
        return (
            <UpperSection>
                {walletView === WALLET_VIEWS.ACCOUNT ? (
                    ...
                    <HeaderRow>
                        <span className={'text-danger wallet-modal__header'}
                            onClick={() => {
                                setPendingError(false)
                    ...
                    <HeaderRow>
                        <span className={'text-primary wallet-modal__header'}
                            onClick={() => {
                                setSelected(undefined);
                                setPendingError(false)
                                setWalletView(WALLET_VIEWS.ACCOUNT)
        ...
        >
            <Modal.Header closeButton>
                {walletView === WALLET_VIEWS.LEDGER_PATH && (
                    ...
                    <button className={'btn btn-light-primary mr-4 d-none d-xl-block'}
                            onClick={() => {
                                setPendingError(false)
                                setWalletView(WALLET_VIEWS.ACCOUNT)
                                setSelected(undefined);
                ...
                <Modal.Title>{walletView === WALLET_VIEWS.LEDGER_PATH ? 'Select HD Derivation path' : account && walletView === WALLET_VIEWS.ACCOUNT ? 'Account' : 'Connect to Wallet' }</Modal.Title>
                {account && walletView === WALLET_VIEWS.ACCOUNT && (
                    <button className={'btn btn-outline-primary ml-auto mr-2 d-none d-xl-block'}
                            onClick={() => {
