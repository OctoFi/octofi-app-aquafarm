...
const OptionCardClickable = styled(OptionCard as any)<{ clickable?: boolean, isRow?: boolean, name?: string | number, selected?: string | undefined }>`
  margin-top: 0;
...
export default function Option({
  ...
  useEffect(() => {
    let tempColor = active ? '#4AC8AA' : !selected || (!!selected && selected !== name) ? '#a890fe' : error ? 'danger' : 'success';
    setLoadingColor(tempColor);
  }, [selected, name, error]);

  const content = (
    <OptionCardClickable id={id} onClick={onClick} clickable={clickable && !active} active={active} selected={selected} name={name}>
      <IconWrapper size={36}>
