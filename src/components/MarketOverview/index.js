...
const PriceChange = styled.span`
  ...
  padding: 5px 15px;
  font-size: 1rem;
  background-color: ${({variant}) => !variant || variant === 'success' ? 'rgba(74, 200, 170, 0.15)' : 'rgba(235, 107, 107, 0.15)'};
  color: ${({ theme, variant }) => variant ? theme[variant] : theme.success};
  border-radius: 12px;
...
const MarketOverview = ({ pair, price, latestPrice, variant, volume, changePercent, row, ...props}) => {
    return (
        <Wrapper>
            <Row>
                <Text fontSize={'.875rem'}>{row ? pair : (
                    <Skeleton width={100} height={24}/>
                )}</Text>
            </Row>
            <Row>
                <Value variant={variant}>{row ? price : (
            ...
            <Row>
                ...
                {row ? (
                    <Text  fontSize={'1rem'}>Volume: {volume}</Text>
                ) : (
