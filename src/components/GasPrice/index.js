    ...
    return (
        <CustomRow className={'custom-row'}>
            {Array.isArray(props.gasList) && props.gasList.map((item) => {
                ...
                return (
                    <Col xs={12} md={6} lg={3}>
                        <GasButton onClick={() => changeSelectedGas(gas)} key={gas} selected={gas === props.selected} className={'mb-3'}>
                            <GasName className={'font-weight-bolder font-size-lg'}>
                                {gas}
                            </GasName>
                            <GasValue className="opacity-70 font-size-sm">
