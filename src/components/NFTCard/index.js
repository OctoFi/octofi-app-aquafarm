...
const NFTCard = props => {
    ...
    const buyHandler = async (e) => {
        ...
        if(account) {
            if(seaport) {
                try {
                    refresh(true);
                    await seaport.fulfillOrder({ order, accountAddress: account, referrerAddress: NFT_REFERRER_ACCOUNT })
    ...
    return (
        <Wrapper>
            <SkeletonTheme color={darkMode ? '#232429' : 'rgb(212, 218, 242)'} highlightColor={darkMode ? '#2b2f36' : '#F3F5FD'}>
            <ImageContainer>
                {loading ? (
